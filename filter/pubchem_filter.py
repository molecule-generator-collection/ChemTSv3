...
class Neutralizer:
    ...
    def neutralise_charges(self, mol, reactions=None):
        replaced = False
...
class Evaluater:
    ...
    def evaluate(self, mol):
        self.det_unknown_atoms(mol)
        self.det_invalid_bonds(mol)
        self.det_invalid_atoms(mol)
    ...
    def det_unknown_atoms(self, mol):
        ctrue = 0
        cfalse = 0
        a_list = []
    ...
    def det_invalid_bonds(self, mol):
        aE_list = self.aEstateMol(mol)
        bonds = mol.GetBonds()
        a_string = ""
        invalid_atoms = []
    ...
    def det_invalid_atoms(self, mol):
        aE_list = self.aEstateMol(mol)
...
class PubChemFilter(MolFilter):
    ...
    def mol_check(self, mol):
        try:
            mol1 = NEUTRALIZER.neutralise_charges(mol)
